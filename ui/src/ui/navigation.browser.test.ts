...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/pronetheia/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/pronetheia");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/pronetheia/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__PRONETHEIA_CONTROL_UI_BASE_PATH__ = "/pronetheia";
    const app = mountApp("/pronetheia/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/pronetheia");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/pronetheia/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "pronetheia.control.settings.v1",
