...
export async function handleWhatsAppStart(host: PronetheiaApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: PronetheiaApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: PronetheiaApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: PronetheiaApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: PronetheiaApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: PronetheiaApp): string {
...
function resolveGatewayHttpAuthHeader(host: PronetheiaApp): string | null {
...
function buildGatewayHttpHeaders(host: PronetheiaApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: PronetheiaApp) {
...
export function handleNostrProfileToggleAdvanced(host: PronetheiaApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: PronetheiaApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: PronetheiaApp) {
  const state = host.nostrProfileFormState;
