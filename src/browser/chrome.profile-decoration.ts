...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".pronetheia-profile-decorated");
}
...
export function decorateOpenClawProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_PRONETHEIA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_PRONETHEIA_BROWSER_COLOR).toUpperCase();
