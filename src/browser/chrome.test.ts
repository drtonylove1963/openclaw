...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateOpenClawProfile(userDataDir, { color: DEFAULT_PRONETHEIA_BROWSER_COLOR });

    const expectedSignedArgb = ((0xff << 24) | 0xff4500) >> 0;

    const def = await readDefaultProfileFromLocalState(userDataDir);

    expect(def.name).toBe(DEFAULT_PRONETHEIA_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_PRONETHEIA_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".pronetheia-profile-decorated"),
      "utf-8",
    );
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_PRONETHEIA_BROWSER_PROFILE_NAME);
  });
});
