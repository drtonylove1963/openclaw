...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.PRONETHEIA_GATEWAY_PORT = prevGatewayPort;
}
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.PRONETHEIA_GATEWAY_PORT;
  process.env.PRONETHEIA_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.PRONETHEIA_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.PRONETHEIA_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.PRONETHEIA_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.PRONETHEIA_GATEWAY_PASSWORD;
  } else {
    process.env.PRONETHEIA_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
