...
const PRONETHEIA_RELAY_BROWSER = "OpenClaw/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.PRONETHEIA_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or PRONETHEIA_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedOpenClawRelay(params: {
  ...
  try {
    ...
    return browserName === PRONETHEIA_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
