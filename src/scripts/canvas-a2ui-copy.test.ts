...
describe("canvas a2ui copy", () => {
  ...
  it("skips missing assets when PRONETHEIA_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      const previous = process.env.PRONETHEIA_A2UI_SKIP_MISSING;
      process.env.PRONETHEIA_A2UI_SKIP_MISSING = "1";
      try {
        await expect(
        ...
        if (previous === undefined) {
          delete process.env.PRONETHEIA_A2UI_SKIP_MISSING;
        } else {
          process.env.PRONETHEIA_A2UI_SKIP_MISSING = previous;
        }
      }
    });
  });
