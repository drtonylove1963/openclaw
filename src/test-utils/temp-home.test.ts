...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.PRONETHEIA_STATE_DIR;
    ...
    expect(process.env.PRONETHEIA_STATE_DIR).toBe(path.join(tempHome.home, ".pronetheia"));
    ...
    expect(process.env.PRONETHEIA_STATE_DIR).toBe(previousStateDir);
