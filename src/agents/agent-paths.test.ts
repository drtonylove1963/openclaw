...
describe("resolveOpenClawAgentDir", () => {
      ...
      withEnv(
        {
          ...
          PRONETHEIA_AGENT_DIR: undefined,
  ...
  it("honors PRONETHEIA_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          PRONETHEIA_STATE_DIR: undefined,
          PRONETHEIA_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveOpenClawAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when PRONETHEIA_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers PRONETHEIA_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          PRONETHEIA_STATE_DIR: undefined,
          PRONETHEIA_AGENT_DIR: primaryOverride,
