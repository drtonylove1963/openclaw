...
import {
  ...
  DEFAULT_PRONETHEIA_BROWSER_COLOR,
  DEFAULT_PRONETHEIA_BROWSER_PROFILE_NAME,
...
const CDP_SOURCE_RANGE_ENV_KEY = "PRONETHEIA_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_PRONETHEIA_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_PRONETHEIA_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_PRONETHEIA_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!hasContainer) {
    ...
    args.push("-e", `PRONETHEIA_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `PRONETHEIA_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `PRONETHEIA_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `PRONETHEIA_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `PRONETHEIA_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_PRONETHEIA_BROWSER_PROFILE_NAME)
    : null;
