...
describe("resolveAgentConfig", () => {
  ...
  it("uses PRONETHEIA_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("PRONETHEIA_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as OpenClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".pronetheia", "workspace"));
  });

  it("uses PRONETHEIA_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("PRONETHEIA_HOME", home);
    // Clear state dir so it falls back to PRONETHEIA_HOME
    vi.stubEnv("PRONETHEIA_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as OpenClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".pronetheia", "agents", "main", "agent"));
  });
});
