...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.PRONETHEIA_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".pronetheia", `workspace-${profile}`);
  }
  return path.join(home, ".pronetheia", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".pronetheia";
