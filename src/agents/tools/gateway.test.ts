...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openclaw: process.env.PRONETHEIA_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.pronetheia === undefined) {
      delete process.env.PRONETHEIA_GATEWAY_TOKEN;
    } else {
      process.env.PRONETHEIA_GATEWAY_TOKEN = envSnapshot.pronetheia;
    }
  ...
  it("uses PRONETHEIA_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.PRONETHEIA_GATEWAY_TOKEN = "local-env-token";
