...
const GOOGLE_LIVE_SINGLE_KEY = "PRONETHEIA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "PRONETHEIA_LIVE_ANTHROPIC_KEY",
    listVar: "PRONETHEIA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "PRONETHEIA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `PRONETHEIA_LIVE_${base}_KEY`;
