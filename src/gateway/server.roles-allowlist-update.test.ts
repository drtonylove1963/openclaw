...
const connectNodeClient = async (params: {
  ...
  const token = process.env.PRONETHEIA_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("PRONETHEIA_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".pronetheia", "restart-sentinel.json");
