...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["PRONETHEIA_GATEWAY_TOKEN", "PRONETHEIA_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.PRONETHEIA_GATEWAY_TOKEN = "env-token";
    process.env.PRONETHEIA_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "PRONETHEIA_GATEWAY_PASSWORD",
      envValue: "from-env",
    ...
    },
    {
      label: "token",
      authKey: "token",
      envKey: "PRONETHEIA_GATEWAY_TOKEN",
      envValue: "env-token",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["PRONETHEIA_GATEWAY_PASSWORD", "PRONETHEIA_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.PRONETHEIA_GATEWAY_PASSWORD = envPassword;
    }
