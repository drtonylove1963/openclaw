  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (PRONETHEIA_SKIP_GMAIL_WATCHER=1)"),
      });
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.PRONETHEIA_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.PRONETHEIA_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (PRONETHEIA_SKIP_CHANNELS=1 or PRONETHEIA_SKIP_PROVIDERS=1)",
        );
      } else {
