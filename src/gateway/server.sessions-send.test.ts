...
beforeAll(async () => {
  envSnapshot = captureEnv(["PRONETHEIA_GATEWAY_PORT", "PRONETHEIA_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.PRONETHEIA_GATEWAY_PORT = String(gatewayPort);
  process.env.PRONETHEIA_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      const configPath = process.env.PRONETHEIA_CONFIG_PATH;
      if (!configPath) {
        throw new Error("PRONETHEIA_CONFIG_PATH missing in gateway test environment");
      }
