...
describe("resolveGatewayConnection", () => {
  ...
  it("uses PRONETHEIA_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ PRONETHEIA_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers PRONETHEIA_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ PRONETHEIA_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
