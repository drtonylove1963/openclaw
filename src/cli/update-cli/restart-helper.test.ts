...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("uses PRONETHEIA_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        PRONETHEIA_PROFILE: "default",
        PRONETHEIA_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.pronetheia.gateway'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses PRONETHEIA_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        PRONETHEIA_PROFILE: "default",
        PRONETHEIA_LAUNCHD_LABEL: "com.custom.pronetheia",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.pronetheia'");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses PRONETHEIA_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        PRONETHEIA_PROFILE: "default",
        PRONETHEIA_WINDOWS_TASK_NAME: "OpenClaw Gateway (custom)",
      });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        PRONETHEIA_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.pronetheia.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("escapes single quotes in profile names for shell scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        PRONETHEIA_PROFILE: "it's-a-test",
      });
    ...
    it("rejects unsafe batch profile names on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = await prepareRestartScript({
        PRONETHEIA_PROFILE: "test&whoami",
      });
