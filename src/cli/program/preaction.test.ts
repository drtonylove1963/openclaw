...
beforeEach(() => {
  ...
  originalHideBanner = process.env.PRONETHEIA_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.PRONETHEIA_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.PRONETHEIA_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  it("hides banner when PRONETHEIA_HIDE_BANNER is truthy", async () => {
    process.env.PRONETHEIA_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
