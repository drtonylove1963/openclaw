...
describe("writeOAuthCredentials", () => {
  const lifecycle = createAuthTestLifecycle([
    ...
    "PRONETHEIA_AGENT_DIR",
    "PI_CODING_AGENT_DIR",
    "PRONETHEIA_OAUTH_DIR",
  ]);
  ...
  it("writes auth-profiles.json under PRONETHEIA_AGENT_DIR when set", async () => {
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    ...
    process.env.PRONETHEIA_AGENT_DIR = kidAgentDir;
  ...
  it("syncs siblings from explicit agentDir outside PRONETHEIA_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-oauth-external-"));
    process.env.PRONETHEIA_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* PRONETHEIA_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to PRONETHEIA_AGENT_DIR when set", async () => {
