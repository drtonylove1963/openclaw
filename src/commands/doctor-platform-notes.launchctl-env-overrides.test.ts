...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "PRONETHEIA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("PRONETHEIA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv PRONETHEIA_GATEWAY_TOKEN");
    expect(message).not.toContain("PRONETHEIA_GATEWAY_PASSWORD");
  });
