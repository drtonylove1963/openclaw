...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.PRONETHEIA_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.PRONETHEIA_GATEWAY_PASSWORD;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.pronetheia.ai/gateway/health",
        "https://docs.pronetheia.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
...
export async function runConfigureWizard(
  ...
  try {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.pronetheia.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.PRONETHEIA_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.PRONETHEIA_GATEWAY_PASSWORD,
    });
    ...
    let gatewayToken: string | undefined =
      ...
      process.env.PRONETHEIA_GATEWAY_TOKEN;
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.PRONETHEIA_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.PRONETHEIA_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.PRONETHEIA_GATEWAY_TOKEN;
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.pronetheia.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
