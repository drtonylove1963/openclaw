...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".pronetheia");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.PRONETHEIA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$PRONETHEIA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via PRONETHEIA_STATE_DIR.
