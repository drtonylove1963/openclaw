...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ PRONETHEIA_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.PRONETHEIA_BASE_ENV ?? "") + "|" + (process.env.PRONETHEIA_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { PRONETHEIA_TEST_ENV: "ok" },
        },
      );
