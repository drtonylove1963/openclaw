...
export function triggerOpenClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.PRONETHEIA_SYSTEMD_UNIT,
        process.env.PRONETHEIA_PROFILE,
      );
  ...
  const label =
    process.env.PRONETHEIA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.PRONETHEIA_PROFILE);
