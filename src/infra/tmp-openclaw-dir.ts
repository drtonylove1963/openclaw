...
export const POSIX_PRONETHEIA_TMP_DIR = "/tmp/openclaw";
...
export function resolvePreferredOpenClawTmpDir(
  ...
  const resolvePreferredState = (
    ...
    try {
      const preferred = lstatSync(POSIX_PRONETHEIA_TMP_DIR);
      if (!isTrustedPreferredDir(preferred)) {
        return "invalid";
      }
      if (requireWritableAccess) {
        accessSync(POSIX_PRONETHEIA_TMP_DIR, fs.constants.W_OK | fs.constants.X_OK);
      }
      return "available";
    } catch (err) {
  ...
  if (existingPreferredState === "available") {
    return POSIX_PRONETHEIA_TMP_DIR;
  }
  ...
  try {
    ...
    mkdirSync(POSIX_PRONETHEIA_TMP_DIR, { recursive: true, mode: 0o700 });
