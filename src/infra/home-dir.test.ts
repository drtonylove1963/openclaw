...
describe("resolveEffectiveHomeDir", () => {
  it("prefers PRONETHEIA_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands PRONETHEIA_HOME when set to ~", () => {
    const env = {
      PRONETHEIA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for PRONETHEIA_HOME", () => {
    const result = resolveRequiredHomeDir(
      { PRONETHEIA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when PRONETHEIA_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ PRONETHEIA_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { PRONETHEIA_HOME: "/srv/openclaw-home" } as NodeJS.ProcessEnv,
    });
