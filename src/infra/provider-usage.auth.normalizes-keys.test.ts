...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(
    ...
    await fs.mkdir(path.join(base, ".pronetheia", "agents", "main", "sessions"), { recursive: true });
    ...
    process.env.PRONETHEIA_STATE_DIR = path.join(base, ".pronetheia");
  ...
  async function writeAuthProfiles(home: string, profiles: Record<string, unknown>) {
    const agentDir = path.join(home, ".pronetheia", "agents", "main", "agent");
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    const stateDir = path.join(home, ".pronetheia");
