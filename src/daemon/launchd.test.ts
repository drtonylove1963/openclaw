...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.pronetheia.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", PRONETHEIA_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    const env: Record<string, string | undefined> = {
      HOME: "/Users/test",
      PRONETHEIA_PROFILE: "default",
    };
    const repair = await repairLaunchAgentBootstrap({ env });
    expect(repair.ok).toBe(true);

    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const label = "ai.pronetheia.gateway";
...
describe("launchd install", () => {
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.pronetheia.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when PRONETHEIA_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.pronetheia.gateway.plist",
    },
    {
      name: "uses profile-specific label when PRONETHEIA_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", PRONETHEIA_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.pronetheia.jbphoenix.plist",
    },
    {
      name: "prefers PRONETHEIA_LAUNCHD_LABEL over PRONETHEIA_PROFILE",
      env: {
        HOME: "/Users/test",
        PRONETHEIA_PROFILE: "jbphoenix",
        PRONETHEIA_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from PRONETHEIA_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        PRONETHEIA_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty PRONETHEIA_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        PRONETHEIA_PROFILE: "myprofile",
        PRONETHEIA_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.pronetheia.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
