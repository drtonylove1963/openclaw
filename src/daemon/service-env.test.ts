...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.PRONETHEIA_GATEWAY_PORT).toBe("18789");
    expect(env.PRONETHEIA_GATEWAY_TOKEN).toBe("secret");
    expect(env.PRONETHEIA_SERVICE_MARKER).toBe("openclaw");
    expect(env.PRONETHEIA_SERVICE_KIND).toBe("gateway");
    expect(typeof env.PRONETHEIA_SERVICE_VERSION).toBe("string");
    expect(env.PRONETHEIA_SYSTEMD_UNIT).toBe("openclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.PRONETHEIA_LAUNCHD_LABEL).toBe("ai.pronetheia.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", PRONETHEIA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.PRONETHEIA_SYSTEMD_UNIT).toBe("openclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.PRONETHEIA_LAUNCHD_LABEL).toBe("ai.pronetheia.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".pronetheia"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", PRONETHEIA_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".pronetheia-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", PRONETHEIA_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".pronetheia"));
  });

  it("uses PRONETHEIA_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", PRONETHEIA_STATE_DIR: "/var/lib/openclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openclaw"));
  });

  it("expands ~ in PRONETHEIA_STATE_DIR", () => {
    const env = { HOME: "/Users/test", PRONETHEIA_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { PRONETHEIA_STATE_DIR: "C:\\State\\openclaw" };
