...
beforeAll(async () => {
  previousFastTestEnv = process.env.PRONETHEIA_TEST_FAST;
  process.env.PRONETHEIA_TEST_FAST = "1";
  ({ getReplyFromConfig } = await import("./reply.js"));
});
afterAll(() => {
  if (previousFastTestEnv === undefined) {
    delete process.env.PRONETHEIA_TEST_FAST;
    return;
  }
  process.env.PRONETHEIA_TEST_FAST = previousFastTestEnv;
});
